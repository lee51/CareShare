...
grant select, insert on table public.p_invites to authenticated;
...
returns table(id uuid, p_id uuid, created_by uuid, created_at timestamptz, expires_at timestamptz, profile_name text, profile_kind text)
language plpgsql
security definer
as $$
begin
...
begin
  ...
  select p.p_id into target_p_id
  from p_invites p
  ...
  return query select target_p_id;
end;
$$;
